...
begin
  ...
  TextCvtData^.ReadBuffer:= AllocMem(ReadBufferSize);
  TextCvtData^.Bitmap:= {$IF CompilerVersion >= 23}VCL.Graphics.TBitmap.Create;{$ELSE}Graphics.TBitmap.Create;{$ENDIF}
...
{ -Open a BMP file for input }
var
  Bmp: {$IF CompilerVersion >= 23}VCL.Graphics.TBitmap;{$ELSE}Graphics.TBitmaps;{$ENDIF}
begin
  { Open the input file }
  Bmp:= {$IF CompilerVersion >= 23}VCL.Graphics.TBitmap.Create;{$ELSE}Graphics.TBitmap.Create;{$ENDIF}
  try
    try
  ...
  if (Bitmap = 0) and (Cvt^.InBitmap = nil) then Result:= ecBadArgument
    ...
    with PBitmapFaxData(Cvt^.UserData)^ do begin
      if Bitmap = 0 then begin
        DataBitmap:= {$IF CompilerVersion >= 23}VCL.Graphics.TBitmap.Create;{$ELSE}Graphics.TBitmap.Create;{$ENDIF}
        DataBitmap.Assign({$IF CompilerVersion >= 23}VCL.Graphics.TBitmap(Cvt^.InBitmap){$ELSE}Graphics.TBitmap(Cvt^.InBitmap){$ENDIF});
  ...
  begin
    ...
    with Unpack^, Data do begin
      ...
      if Width > 0 then begin
        ...
        for i:= 0 to Pred(Height) do begin
          Move(GetPtr(Lines, Integer(Width) * i)^, OutLine^, Width);
